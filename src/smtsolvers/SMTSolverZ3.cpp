...
SMTSolverZ3::SMTSolverZ3(bool verbose)
    : m_context(), m_solver(m_context), m_assertion_count(0), m_verbose(verbose), m_interrupted(false)
{
    if (m_verbose)
    {
...
// ============================================================
// Convert a single variable name into Term
// ============================================================
Rational SMTSolverZ3::getRationalValue2(const std::string &var_name){
    if (!variableExists(var_name))
    {
        throw std::runtime_error("SMTSolverZ3::getRationalValue2() : variable '" + var_name + "' non declaree");
    }
    z3::model model = m_solver.get_model();
    z3::expr var = getVariable(var_name);
    z3::expr value = model.eval(var, true);
    return const2Rational(z3ExprToTerm(value));
}

std::shared_ptr<Term> SMTSolverZ3::z3ExprToTerm(const z3::expr &value)
{
    // ---- Case 1: numeral leaf (most common path from model.eval) ----
    // In z3++, model.eval() returns real values as rational numerals,
    // NOT as ApplicationTerm "(/ 3 4)" — that is only in text output.
    if (value.is_numeral())
    {
        if (value.is_int())
        {
            // Integer sort: e.g. value 5
            int64_t v = 0;
            if (value.is_numeral_i64(v))
            {
                return Term::makeInt(v);
            }
            // Overflow fallback: parse string (use big-int lib in production)
            std::cout<< "Warning: integer value " << value.to_string() << " too large for int64, using string parsing fallback" << std::endl;
            std::string s = "";
            value.is_numeral(s);
            return Term::makeInt(std::stoll(s));
        }
    ...
    // ---- Case 2: ApplicationTerm (arithmetic expression node) ----
    // Reached when Z3 returns compound expressions, e.g. after
    // simplification or when SMTLIB2_COMPLIANT text output is parsed back.
    if (value.is_app())
    {
        throw std::runtime_error(
            "z3ExprToTerm reached non-numeral application term " +
            value.to_string());
        // const Z3_decl_type type = value.decl().decl_kind();
        // const unsigned nargs = value.num_args();

        // switch (type)
        // {
        // case Z3_OP_UMINUS:
        //     // Unary minus: (- x)
        //     return Term::makeApp("-", {z3ExprToTerm(value.arg(0))});

        // case Z3_OP_ADD:
        // {
        //     // Z3 may produce n-ary ADD; fold left into binary "+"
        //     auto acc = z3ExprToTerm(value.arg(0));
        //     for (unsigned i = 1; i < nargs; ++i)
        //         acc = Term::makeApp("+", {acc, z3ExprToTerm(value.arg(i))});
        //     return acc;
        // }
        ...
        //         return Term::makeApp("-", {z3ExprToTerm(value.arg(0))});
        //     return Term::makeApp("-", {z3ExprToTerm(value.arg(0)),
        //                             z3ExprToTerm(value.arg(1))});

        // case Z3_OP_MUL:
        // {
        //     // Z3 may produce n-ary MUL; fold left into binary "*"
        //     auto acc = z3ExprToTerm(value.arg(0));
        //     for (unsigned i = 1; i < nargs; ++i)
        //         acc = Term::makeApp("*", {acc, z3ExprToTerm(value.arg(i))});
        //     return acc;
        ...
        // case Z3_OP_DIV:  // Real division  (/ a b)
        // case Z3_OP_IDIV: // Integer division (div a b)
        //     return Term::makeApp("/", {z3ExprToTerm(value.arg(0)),
        //                             z3ExprToTerm(value.arg(1))});

        // default:
        //     throw std::runtime_error(
        //         "z3ExprToTerm: unsupported decl_kind " +
        //         std::to_string(static_cast<int>(type)));
        // }
    }

    throw std::runtime_error("z3ExprToTerm: unhandled expr type");
}
...
std::shared_ptr<SMTSolverInterface> SMTSolverZ3::clone() const
{
    ...
    for (const auto &[name, expr] : m_variables)
    {
        ...
        else if (s.is_array())
        {
            ...
            std::string idx_str = idx.is_int() ? "Int" : idx.is_real() ? "Real"
                                                                       : idx.name().str();
            std::string elem_str = elem.is_int() ? "Int" : elem.is_real() ? "Real"
                                                                          : elem.name().str();
    ...
    const std::string &op, const std::vector<z3::expr> &args)
{
    ...
    {
        if (args.empty())
            throw std::runtime_error("+ requires at least 1 argument");
        if (args.size() == 1)
            return args[0];
    ...
    {
        if (args.size() != 2)
            throw std::runtime_error("div requires exactly 2 arguments");
    ...
    {
        if (args.size() != 2)
            throw std::runtime_error("mod requires exactly 2 arguments");
    ...
    {
        if (args.empty())
            throw std::runtime_error("and requires at least 1 argument");
        if (args.size() == 1)
            return args[0];
        z3::expr result = args[0];
        for (size_t i = 1; i < args.size(); ++i)
    ...
    {
        if (args.empty())
            throw std::runtime_error("or requires at least 1 argument");
        if (args.size() == 1)
            return args[0];
        z3::expr result = args[0];
        for (size_t i = 1; i < args.size(); ++i)
    ...
    {
        if (args.size() != 1)
            throw std::runtime_error("not requires exactly 1 argument");
        return !args[0];
    }
    ...
    {
        if (args.size() != 3)
            throw std::runtime_error("ite requires exactly 3 arguments");
    ...
    {
        if (args.size() < 2)
            throw std::runtime_error("distinct requires at least 2 arguments");
    ...
    {
        if (args.size() != 2)
            throw std::runtime_error("select requires exactly 2 arguments");
    ...
    {
        if (args.size() != 3)
            throw std::runtime_error("store requires exactly 3 arguments");
